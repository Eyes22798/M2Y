...
| 21 | Shared Item 的类型集合在**三层各不相同**，且最宽的一层不可存储（2026-08-25 新发现，本轮补记） | PRD `M2Y_PRD_修订版.md:1474` = `'pin' \| 'task' \| 'note' \| 'file' \| 'agreement'`；`CLAUDE.md:27` 原文照抄该 5 项；`src/domain/shared-item/types.ts:1` 的 `sharedItemKinds` = `['task','note','file','agreement','event']`（**无 `pin`、多出文档从未提及的 `event`**）；`types.ts:3` 的 `previewSharedItemKinds` 与 `src/data/sqlite/schema-v1.ts:20` 的 `CHECK(kind IN ('note','task','agreement'))` = 3 项。写路径 `SaveToSpaceSheet.tsx:25` 与读路径 `row-decoders.ts:78,108` 都只认这 3 项，所以 **`file`/`event` 是类型上合法、写库必被 CHECK 拒绝的不可达声明**，唯一消费宽联合的 `shared-item-presenters.ts:37` 因此带着两条永不执行的分支。当前不构成运行时缺陷（没有任何路径能构造这两个值），但正是「读文档写代码」会踩的坑：按 `CLAUDE.md` 去实现 `pin` 会在 insert 处炸。待产品决策：`event` 是保留还是删除、`pin` 何时进 schema |
